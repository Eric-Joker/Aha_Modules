...
EVENT_PROB = cfg.get_config("event_prob", 0.05, "随机事件总触发概率。")
STREAK_BONUS_CYCLE = cfg.get_config("streak_cycle", 7, "连续签到周期。")
STREAK_BONUS_STAGES = cfg.get_config("streak_stages", 6, "固定周期次数。")
STREAK_BONUS_MAX = cfg.get_config("streak_max", 3, "固定周期奖励上限。")
STREAK_BONUS_RANGE = cfg.get_config("streak_range", (5, 10), "随机周期范围。")
STREAK_BONUS_PONITS = cfg.get_config("streak_points", (1, 15), "随机周期奖励范围。")
...
async def sign(user_id, nickname):
    ...
    return f"{nickname} 签到成功，当前持有 {decimal_to_str(round_decimal(await inquiry_point(user_id)))}+{await adjust_point(user_id, points)} 点。"
