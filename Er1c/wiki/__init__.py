...
async def fetch(event: Message, match: Match):
    if not (url := WIKI_MAP.get(match[1])):
        return

    await API.poke()

    try:
        if result := await (client := MediaWikiClient(url)).fetch_intro(term := match[2].strip()):
...
async def reget(event: Message, match: Match):
    ...
    try:
        if result := await MediaWikiClient().get_cached_intro(await event.user_aha_id(), int(match[1]) - 1):
            await send_response(event, result)
    except Exception:
        await handle_error(event)
